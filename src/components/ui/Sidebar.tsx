...
export function Sidebar({ programName }: { programName: string }) {
  ...
  return (
    <nav className="dash-sidebar">
      <div style={{ fontWeight: 700, fontSize: 15 }}>strategos</div>
      <div className="dash-nav">
        {NAV.map((n) => {
          const on = n.href === "/" ? pathname === "/" : pathname.startsWith(n.href);
          return (
            <a key={n.key} href={n.href}
               style={{ padding: "7px 10px", borderRadius: "var(--radius-sm)", fontSize: 13, whiteSpace: "nowrap",
      ...
      <div className="dash-program" style={{ marginTop: "auto", padding: 10, background: "var(--surface-2)", borderRadius: "var(--radius-sm)" }}>
